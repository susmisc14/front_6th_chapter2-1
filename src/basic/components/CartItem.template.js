...
import { renderPriceLabel, renderNamePrefix } from './PriceLabel.template.js';
...
export function createCartItemElement(item) {
  return createElementsFromTemplate(`
    <div id="${item.id}" class="grid grid-cols-[80px_1fr_auto] gap-5 py-5 border-b border-gray-100 first:pt-0 last:border-b-0 last:pb-0">
      ...
      <div>
        <h3 class="text-base font-normal mb-1 tracking-tight">${renderNamePrefix(item)}${item.name}</h3>
        <p class="text-xs text-gray-500 mb-0.5 tracking-wide">PRODUCT</p>
        <p class="text-xs text-black mb-3">${renderPriceLabel(item)}</p>
      ...
      <div class="text-right">
        <div class="text-lg mb-2 tracking-tight tabular-nums">${renderPriceLabel(item)}</div>
